...
const BoxCell = React.memo(({ showPlus }: { showPlus: boolean }) => (
    ...
    >
        {showPlus && (
            <svg
                className="pointer-events-none absolute -left-[22px] -top-[14px] h-6 w-10 text-slate-500 dark:text-slate-600"
                fill="none"
                stroke="currentColor"
                strokeWidth="1.5"
                viewBox="0 0 24 24"
...
const BoxRow = React.memo(({ rowIndex, cols }: { rowIndex: number; cols: number }) => (
    <div className="relative h-8 w-16 border-l border-slate-400 dark:border-slate-700">
